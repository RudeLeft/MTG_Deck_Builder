...
| `mtgdb/ui/table_filters.py` | change numeric/text/categorical/mana-cost-symbol filter editor behavior or deck-table filter adaptation<br>change async Results vocabulary popup integration, sorting actions, or lifecycle | `mtgdb/ui/tables.py`; `mtgdb/preferences/repository.py`; owning table screen | Shared column definitions/formatters belong in `ui/tables.py`; the Cost column filters by mana-symbol group via the shared `cost_symbol_groups` in `search/results.py` so the popup and matcher never diverge. |
